...
import com.customized.support.security.BASE64Encoder;
import com.customized.support.security.coder.*;
...
public final class SecurityUtil {
	 ...
	 * @return 返回密文
	 */
	public static final String encryptMd5toKen(String strSrc) {
	...
	public static final String signRSA(String data, String privateKey) {
		try {
			return RSACoder.sign(CHARSET, decryptBASE64(privateKey));
		} catch (Exception e) {
	...
	public static final boolean verifyRSA(String data, String publicKey, String sign) {
		try {
			return RSACoder.verify(CHARSET, decryptBASE64(publicKey), sign);
		} catch (Exception e) {
	...
	public static final String encryptRSAPrivate(String data, String privateKey) {
		try {
			return RSACoder.encryptByPrivateKey(CHARSET, decryptBASE64(privateKey));
		} catch (Exception e) {
			throw new RuntimeException("解密错误，错误信息：", e);
		}
	}
	...
	public static final String decryptRSAPublic(String cryptData, String publicKey) {
		try {
			// 把字符串解码为字节数组，并解密
			return new String(RSACoder.decryptByPublicKey(cryptData, decryptBASE64(publicKey)));
		} catch (Exception e) {
			throw new RuntimeException("解密错误，错误信息：", e);
		}
	}
